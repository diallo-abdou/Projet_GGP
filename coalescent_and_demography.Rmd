...
THETA <- 1
NLOCI <- 100
model_bottleneck <- coal_model(sample_size =  20,loci_number =  NLOCI,loci_length =  NLOCI,ploidy = 2)+
...
set.seed(123) #correspond à la première simulations dans la boucle de 30
...
```{r}
ggplot()+
  geom_histogram(aes(x = sim$pi),color = "black", fill = "white", binwidth = 0.2)+
  labs(title = paste("Mean pi = ",round(obs$mean_pi,2)," +/- ",round(1.96*obs$se_pi,2)))+
  xlab("pi")+
  theme_classic()

ggplot()+
  geom_histogram(aes(x = sim$Dtaj),color = "black", fill = "white", binwidth = 0.5)+
  labs(title = paste("Mean Dtaj =",round(obs$mean_Dtaj,2),"+/-",round(1.96*obs$se_Dtaj,2)))+
  xlab("Dtaj")+
  theme_classic()

ggplot()+
  geom_histogram(aes(x = sim$sfs),color = "black", fill = "white",binwidth = 5)+
  labs(title = "SFS")+
  xlab("Allele frequency")+
  theme_classic()

tr <- read.tree(text = sim$trees[[7]][[1]])
plot(tr,main="Example of a genealogy",show.tip.label = FALSE,x.lim = c(0,0.4))

ggplot()+
  geom_histogram(aes(x = coal_time),color = "black", fill = "white",binwidth = 0.1)+
  labs(title = paste("Mean time to MRCA =",round(mean(coal_time),2),"+/-",round(1.96*sd(coal_time)/sqrt(NLOCI),2)))+
  xlab("Time to MRCA")+
  theme_classic()
...
## bottleneck avec 30 simulations
```{r}
...
model <- coal_model(sample_size =  20,loci_number =  NLOCI,loci_length =  NLOCI,ploidy = 2)+
  feat_mutation(rate = THETA)+
  feat_size_change(0.1, time = 0.25) +
  feat_size_change(1, time = 0.5)+
  sumstat_sfs("sfs")+
...
ggplot()+
  geom_histogram(aes(x = tot_obs$mean_pi),color = "black", fill = "white",binwidth = 0.02)+
  labs(title = "Mean pi distribution")+
  xlab("mean pi")+
  theme_classic()

ggplot()+
  geom_histogram(aes(x = tot_obs$mean_Dtaj),color = "black", fill = "white",binwidth = 0.05)+
  labs(title = "Mean Dtaj distribution")+
  xlab("mean Dtaj")+
  theme_classic()

ggplot()+
  geom_histogram(aes(x = tot_obs$mean_coal_time),color = "black", fill = "white",binwidth = 0.02)+
  labs(title = "Mean time to MRCA distribution")+
  xlab("Mean time to MRCA")+
  theme_classic()


```

# sans évolution de la taille de population
```{r}
n_obs=30
tot_obs_no_change <- data.frame(mean_pi = character(),
                 mean_Dtaj = character(),
                 mean_coal_time = character())
...
model2 <- coal_model(sample_size =  20,loci_number =  NLOCI,loci_length =  NLOCI,ploidy = 2)+
  feat_mutation(rate = THETA)+
  sumstat_sfs("sfs")+
  sumstat_tajimas_d(name = "Dtaj") +
  sumstat_nucleotide_div(name = "pi")+
  sumstat_trees(name="trees") 
set.seed(i)
sim2 <- simulate(model2)

mean_pi <- mean(sim2$pi)
#se_pi <- sd(sim$pi)/sqrt(NLOCI)
mean_Dtaj <- mean(sim2$Dtaj,na.rm = T) # Dtaj is not defined if there is no SNPs
#se_Dtaj <- var(sim$Dtaj,na.rm = T)/sqrt(NLOCI)
coal_time_temp <- rep(NA,100)
for (i in 1:NLOCI) {
...
obs_no_change <- data.frame(mean_pi = mean(sim2$pi), 
                 mean_Dtaj = mean(sim2$Dtaj,na.rm = T),
                 mean_coal_time = mean(coal_time_temp,na.rm = T)
                 )

tot_obs_no_change=rbind(tot_obs_no_change,obs_no_change)

}
tot_obs_no_change

ggplot()+
  geom_histogram(aes(x = tot_obs_no_change$mean_pi),color = "black", fill = "white",bins = 10)+
  labs(title = "Mean pi distribution")+
  xlab("mean pi")+
  theme_classic()

ggplot()+
  geom_histogram(aes(x = tot_obs_no_change$mean_Dtaj),color = "black", fill = "white",bins = 10)+
  labs(title = "Mean Dtaj distribution")+
  xlab("mean Dtaj")+
  theme_classic()

ggplot()+
  geom_histogram(aes(x = tot_obs_no_change$mean_coal_time),color = "black", fill = "white",bins = 10)+
  labs(title = "Mean time to MRCA distribution")+
  xlab("Mean time to MRCA")+
  theme_classic()
...
```{r}
dist = apply((tot_obs_no_change[,c("mean_pi", "mean_Dtaj","mean_coal_time")]/tot_obs[,c("mean_pi","mean_Dtaj","mean_coal_time")] -1 )^2,MARGIN = 1,function(x) sqrt(sum(x)))

ggplot()+
  geom_histogram(aes(x = dist),color = "black", fill = "white",bins = 10)+
  labs(title = paste("Mean Distance between two simulations = ",round(dist,2)," +/- ",round(1.96*sd(dist)/sqrt(length(dist)),2)))+
  xlab("Distance between two simulations")+
  theme_classic()
